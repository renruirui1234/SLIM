...
class FeatureBinarizer(TransformerMixin):
    ...
    '''
    def __init__(self, colCateg=[], numThresh=99, negations=False, threshStr=False, returnOrd=False, **kwargs):
        """
        Args:
    ...
    def fit(self, X):
        ...
        # quantProb = np.concatenate((quantProb[:5],quantProb[-10:]))
        # quantProb = quantProb[-10:]#效果还行
        # quantProb = np.concatenate((quantProb[[5,10,15,20,25,30,35,40,45,50]],quantProb[-10:]))
        # Initialize
        maps = {}
        enc = {}
        thresh = {}
    ...
    def transform(self, X):
        ...
        for c in data:
            ...
            # Categorical column
                ...
                Anew = pd.DataFrame(Anew, columns=enc[c].categories_[0].astype(str))
                if self.negations:
                    # Append negations
        ...
        if self.returnOrd:
            ...
            Xstd = pd.DataFrame(Xstd, columns=self.ordinal)
